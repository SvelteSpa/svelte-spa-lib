...
type Options = {
  tabs: int
  indentOn: null | RegExp
  spellcheck: boolean
  catchTab: boolean
  preserveIdent: boolean
  addClosing: boolean
  msHighlight: number // debounce ms
  history: boolean
...
export function CodeJar(
  ...
  // function var z = (i)=>Math.max(0,Math.trunc((i-1)/2)*2);
  // function
  // z = (i)=>Math.trunc((i+2)/2)*2;
  ...
  function handleNewLine(event: KeyboardEvent) {
    if (event.key === 'Enter') {
      ...
      if (options.indentOn?.test(before)) {
        newLinePadding += ' '.repeat(options.tabs)
      }
  ...
  function addPad(start: number) {
    let ts = options.tabs
    let stop = Math.trunc(start / ts) * ts
    let pad = ts - (start - stop)
    return ' '.repeat(pad)
  }
